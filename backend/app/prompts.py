...
5. 不要把课程知识点自动扩展成先修数学薄弱点；只有学生明确说“概率论/线性代数/Python/高数”等基础薄弱时，才写入对应 weak_points 或 knowledge_base。
6. 如果学生明确列出“当前困惑/比较困惑”的知识点，weak_points 应优先记录这些原始课程知识点，例如 A*搜索、过拟合、Transformer自注意力、RAG、Top-K检索、重排序。
...
5. 当前薄弱点优先：如果最新对话明确列出“我现在对/我对/比较困惑”的课程知识点，应将这些知识点作为当前 weak_points 的优先内容；旧画像中未被本轮提及的“概率论、线性代数、Python、高数”等先修基础薄弱点不要继续放在当前 weak_points，除非学生本轮明确提到这些基础薄弱。
6. 禁止过度推断：不要因为学生提到过拟合、RAG、Transformer、自注意力、Top-K、重排序，就推断其需要学习概率论或线性代数。
7. 字段约束：profile 字段必须是符合 Profile Pydantic 模型的完整 JSON 对象，字段名、字段类型必须一致。
8. 输出约束：只输出 JSON 对象，不要 Markdown，不要解释文字，不要代码块。
